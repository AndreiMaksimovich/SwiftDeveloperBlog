...
enum AppDataSchemaV04: VersionedSchema {
    ...
    class Path {
        ...
        var someUselessAISummary: String?
        
        init(uid: UUID = .init(), title: String, commnent: String, duration: TimeInterval?, points: [PathPoint], someUselessAISummary: String = "") {
            self.uid = uid
            self.title = title
            self.comment = commnent
            self.points = points
            self.duration = duration
            self.someUselessAISummary = someUselessAISummary
        }
    }
